...
class RPMTransaction(object):
    ...
    def _transStart(self, bytes, total, h):
        ...
        self.ts_done_open()
        self._te_list = list(self.base.ts)
    ...
    def _elemProgress(self, bytes, total, h):
        # This callback type is issued every time the next transaction element
        # is about to be processed by RPM and before any other callbacks are
        # issued.  The index of the element is also passed so we don't have to
